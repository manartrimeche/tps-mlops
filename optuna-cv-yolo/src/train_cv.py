...
def main():
    ...
    with mlflow.start_run(run_name=args.exp_name):
        ...
        # Sanitize metric names: MLflow doesn't allow parentheses in metric names
        for key in ["metrics/mAP50(B)", "metrics/mAP50-95(B)", "metrics/precision(B)", "metrics/recall(B)"]:
            if key in metrics:
                # Replace parentheses with underscores for MLflow compatibility
                sanitized_key = key.replace("(", "_").replace(")", "")
                log_metric(sanitized_key, float(metrics[key]))
